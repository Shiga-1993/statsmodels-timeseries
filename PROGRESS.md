...
## 観測まとめ
- **Prophet**
  - 13銘柄のうち9銘柄が乗法季節性を選択しており、季節性・トレンド調整でほぼ説明できる。N225を外生に入れてもラグごとのスコア差はごく僅少（lag 0〜2でほぼ横並び）で、外生より季節性チューニングが優先という結論。
  - N225を1ヶ月遅行させても RMSE 改善は 3/13 銘柄のみで、多くは係数が不安定になった。現時点では Prophet における N225の説明力は限定的。
  - 対策: リターンの標準化、レベルターゲット検証、追加共変量導入などスケーリングと仕様見直しを優先する。
- **SARIMAX**
  - 10/13 銘柄が「N225ログリターンの1ヶ月遅行 (lag=1)」を CV で最良と判定し、係数も有意 (p<0.05)。つまりN225が翌月リターンへ与える影響は比較的大きい。
  - lag 6 or 12 が効いたのは一部のみで、係数も小さい。SARIMAXでは lag 1 を起点に細かな調整を行うのが合理的。
  - ConvergenceWarning が散見されるため、`mle_retvals['converged']` チェックやBICによるフィルタリングを推奨。
